...
# show help msg
show_help() {
    echo -e "Usage: $0 [-f xxx | -l | -p xxx | -g [xxx] | -v xxx | -h]"
    echo -e "  -f, --core-file <path>          自定义 $is_core_name 文件路径, e.g., -f /root/$is_core-linux-amd64.tar.gz"
    echo -e "  -l, --local-install             本地获取安装脚本, 使用当前目录"
    echo -e "  -p, --proxy <addr>              使用代理下载, e.g., -p http://127.0.0.1:2333"
    echo -e "  -g, --github-proxy [URL]        使用 GitHub 加速域名, 默认: https://ghproxy.com/, e.g., -g https://mirror.ghproxy.com/"
...
download() {
    ...
    # 若启用 GitHub 加速域名，则拼接前缀
    if [[ -n $github_proxy ]]; then
        link="${github_proxy}${link}"
    fi
...
main() {
    ...
    [[ $github_proxy ]] && msg warn "使用 GitHub 加速域名: ${yellow}$github_proxy${none}"
    # create tmpdir
    mkdir -p $tmpdir
